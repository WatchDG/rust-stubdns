...
async fn handle_tcp_connection(mut socket: TcpStream, client_addr: std::net::SocketAddr, addr: String) {
    println!(
...
pub async fn start_tcp_server(host: String, port: u16) {
    ...
    loop {
        ...
        tokio::spawn(async move {
            handle_tcp_connection(socket, client_addr, addr_clone).await;
        });
    }
}
